...
import com.saltdamage.dto.BlockchainInfoDTO;
import com.saltdamage.dto.BlockchainRecordDTO;
import com.saltdamage.dto.StoreDataRequest;
import com.saltdamage.entity.BlockchainRecordEntity;
import com.saltdamage.repository.BlockchainRecordRepository;
...
public class BlockchainController {
    ...
    @GetMapping("/latest")
    public ApiResponse<BlockchainService.Block> getLatestBlock() {
        log.info("获取最新区块");
        try {
            BlockchainService.Block block = blockchainService.getLatestBlock();
            if (block != null) {
                return ApiResponse.success(block);
            } else {
                return ApiResponse.error(404, "暂无区块数据");
            }
        } catch (Exception e) {
